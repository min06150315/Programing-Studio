...
// 구조체 배열에 입력한 정보들을 같은 폴더에 특정이름을 부여해서 text 파일로 저장해보세요.
...
void savefileScore(Student *s, int count){
    int count = 0;
    FILE *fp = NULL;
...
int loadfileScore(Student *s, int count){
    FILE *fp = NULL;
...
int main(void)
{
    ...
    // Save studnet info in new file
    savefileScore(&s, count);
    loadfileScore(&s, count);

    // Search the name in file
    return 0;
